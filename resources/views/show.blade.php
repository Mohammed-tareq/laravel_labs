...
<x-layouts.guest_layout>
    <div class="bg-gray-100 min-h-screen flex items-center justify-center p-4">
        <div class="w-full max-w-4xl mx-auto bg-white rounded-lg shadow p-6 space-y-10">

            <!-- Post Info -->
            <section>
                <h2 class="text-lg md:text-xl font-semibold text-indigo-600 mb-4">Post Info</h2>
                <div class="space-y-3">
                    <div>
                        <span class="font-medium text-gray-700">Title:  <span class="text-gray-900 break-words">{{ $limitPost->title }}</span></span>

                    </div>
                    <div>
                       <p class="text-gray-700 leading-relaxed"> <span class="font-medium text-gray-700">Description:</span> {{ $limitPost->description }}</p>
                    </div>
                </div>
            </section>
            ...
            <!-- Post Creator Info -->
            <section>
                <h2 class="text-lg md:text-xl font-semibold text-indigo-600 mb-4">Post Creator Info</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="sm:col-span-2">
                        <span class="font-medium text-gray-700">Email:</span>
                        <span class="text-gray-900 break-all">{{ $limitPost->user->email }}</span>
                    </div>
                    <div class="sm:col-span-2">
                        <span class="font-medium text-gray-700">Created At:</span>
                        <span class="text-gray-900">{{ $limitPost->created_at->diffForHumans() }}</span>
            ...
            <!-- Add Comment -->
            <section>
                <h2 class="text-lg md:text-xl font-semibold text-indigo-600 mb-4">Add a Comment</h2>
                <form id="commentForm" class="space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea id="commentText" name="comment" rows="3"
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition resize-none"
                            placeholder="Write your comment..."></textarea>
                    </div>

                    <div>
                        <label for="comment_user" class="block text-sm font-medium text-gray-700 mb-1">Comment As</label>
                        <select id="comment_user" name="user_id"
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition">
                            <option value="" disabled selected>Select User</option>
                            @foreach ($users as $user)
                    ...
                    <div>
                        <button type="submit" id="submit"
                            class="w-full sm:w-auto px-5 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition">Submit</button>
                    </div>
                </form>
            </section>
            ...
            <!-- Comments List -->
            <section>
                <h2 class="text-lg md:text-xl font-semibold text-indigo-600 mb-4">Comments</h2>
                <div class="space-y-4" id="comments">
                    @forelse ($comments as $comment)
                        <div class="border border-gray-200 rounded-lg p-4">
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2">
                                <div class="text-sm text-gray-600">
                                    ...
                                    <span class="block sm:inline">{{ $comment->created_at->diffForHumans() }}</span>
                                </div>
                            </div>
                            <p class="text-gray-800">{{ $comment->comment }}</p>
                        </div>
                    ...
                    @if (!empty($comments))
                        <button type="button" id="ShowMore"
                            class="w-full sm:w-auto px-5 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition">Show More</button>
                    @endif
                </div>
            </section>
        </div>
                                ...
                                <div class="border border-gray-200 rounded-lg p-4 mb-2">
                                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2">
                                        <div class="text-sm text-gray-600">
                                            <span class="font-medium text-gray-800">${comment.user.name}</span>
                                            <span class="mx-2 hidden sm:inline">•</span>
                                            <span class="block sm:inline">${comment.created_at}</span>
                                        </div>
                                    </div>
                                    <p class="text-gray-800">${comment.comment}</p>
                                </div>
